"""Tests for screenshot type detection (Task 020)."""
...
from scoretopia.domain.actions import UnrecognizedScreenshot
...
from scoretopia.screenshot.models import GameEndExtraction
...
def test_detect_screenshot_type_ongoing_list_not_game_end() -> None:
    """Ongoing multiplayer card must not be classified as game_end."""
    with pytest.raises(ValueError):
        detect_screenshot_type(_epic_wasteland_ongoing_ocr_lines())


def test_detect_screenshot_type_ongoing_list_raises_unrecognized_with_guidance() -> (
    None
):
    """Ongoing list should fail detection with guidance, not parse as game start."""
    with pytest.raises(ValueError) as exc_info:
        detect_screenshot_type(_epic_wasteland_ongoing_ocr_lines())

    message = str(exc_info.value).lower()
    assert "ongoing" in message
    assert "game basics" in message or "lobby" in message
...
def test_ingest_ongoing_list_card_returns_unrecognized_with_guidance(
    ingest_service: IngestService,
    tmp_path: Path,
) -> None:
    """Ingest path surfaces ongoing-list guidance instead of staging game_end."""
    ...
    with patch(
        ...
        side_effect=ValueError(ongoing_message),
    ):
